...
import '../../main.dart';
import '../api/model/rich_notification.dart';
...
class NotificationController {
  ...
  static Future<void> handleNotification(ReceivedAction notification) async {
    if (notification.payload != null) {
      ...
      // Navigate to Route
      await MyApp.navigatorKey.currentState?.pushReplacementNamed(routeName,
          arguments: NotificationRouteArguments(actionKey, notif));

      // marking the notification as read
      await bloc.clearNotificationUsingID(notif.notificationID!);

      // Handling action key
      await _handleActionKey(actionKey, notif);
    }
  }

  /// Handle what action to take depending on key
  static Future<void> _handleActionKey(String actionKey, RichNotification notif) async {
    // Open browser
...
Future<void> setupNotifications(BuildContext context, InstiAppBloc bloc) async {
  // Check for permission (if not granted, request it)
  if (await bloc.hasNotificationPermission() == null) {
    await requestNotificationPermission(context, bloc);
  }
